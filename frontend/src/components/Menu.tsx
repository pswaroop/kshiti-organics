...
const Menu: React.FC = () => {
  ...
  useEffect(() => {
    const fetchBackendData = async () => {
      setIsLoading(true);
      try {
        console.log("Fetching data from:", import.meta.env.VITE_API_BASE_URL);
        ...
        if (catRes && catRes.ok) {
          ...
          if (catData.length > 0) {
            ...
            const newIconMap: Record<string, string> = { All: "🌿" };
            catData.forEach((c) => {
              newIconMap[c.name] = c.icon || "📦"; // Fallback icon
            });
            setCategoryIconMap(newIconMap);
          }
        } else {
          console.warn("Using default categories (API unavailable or empty)");
        }

        // Process Products
        if (prodRes && prodRes.ok) {
          const prodData: Product[] = await prodRes.json();
          console.log("Products received:", prodData.length);
          if (prodData.length > 0) {
            setProducts(prodData);
          }
        } else {
          console.warn("Using default products (API unavailable or empty)");
        }
      } catch (error) {
  ...
  const productCounts = useMemo(() => {
    ...
    activeProducts.forEach((product) => {
      if (counts[product.category] !== undefined) {
        ...
        // Handle case where product has a category not in the list
  ...
  // ... rest of your render code (return statement) remains exactly the same ...
  return (
    <section id="products" className="py-16 bg-muted/30 min-h-screen">
      <div className="container mx-auto px-4">
        {/* Section Header */}
